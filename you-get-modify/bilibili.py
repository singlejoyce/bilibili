...
class Bilibili(VideoExtractor):
    ...
    @staticmethod
    def bilibili_space_video_by_search_api(mid, keyword, pn=1, ps=100):
        return 'https://api.bilibili.com/x/space/arc/search?mid=%s&ps=%s&tid=0&pn=%s&keyword=%s&order=pubdate&jsonp=jsonp' % (
        mid, ps, pn, keyword)
    ...
    def download_playlist_by_url(self, url, **kwargs):
        ...
        if sort == 'video':
            ...
            mid = m.group(1)
            keyword = self.url.split('keyword=')[1]
            # 如果url中带有keyword词语证明是搜索关键字类型，不需要下载全部视频，只要下载搜索结果中的相关视频
            if 'keyword' in self.url:
                api_url = self.bilibili_space_video_by_search_api(mid, keyword)
                api_content = get_content(api_url, headers=self.bilibili_headers())
                videos_info = json.loads(api_content)
                pc = round(videos_info['data']['page']['count']/videos_info['data']['page']['ps'])

                for pn in range(1, pc + 1):
                    api_url = self.bilibili_space_video_by_search_api(mid, keyword, pn=pn)
                    api_content = get_content(api_url, headers=self.bilibili_headers())
                    videos_info = json.loads(api_content)
                    ...
                    for video in videos_info['data']['list']['vlist']:
                        ...
                        url = 'https://www.bilibili.com/video/%s' % video['bvid']
                        self.__class__().download_playlist_by_url(url, **kwargs)
            else:
